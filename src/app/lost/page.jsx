...
export default function LostItemsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await fetch("/api/reports?status=lost");
        ...
        if (!cancelled && data.success) {
          ...
          const mapped = data.reports.map((r) => ({
            ...
            // keep both reportType and itemStatus so UI can show left pill (reportType) and right pill (claim/item status)
            reportType: (r.reportType || "").toString(),
            itemStatus: (r.itemStatus || "").toString(),
            status: (r.itemStatus || r.reportType || "lost").toString(),
          }));
          setItems(mapped);
        }
      } catch (e) {
    ...
    async function loadMyClaims() {
      try {
        ...
        if (j && j.success && Array.isArray(j.claims)) {
          ...
          if (!cancelled) setMyClaimsMap(map);
        }
      } catch (e) {
            ...
            >
              ...
              <div className="flex-1">
                <div className="mb-2 flex items-center gap-2">
                  {/* left pill: report type (Lost/Found) or fallback to itemStatus */}
                  <span className="inline-block px-3 py-1 text-xs rounded-full bg-red-100 text-red-700 font-medium">
                    {(item.reportType || item.itemStatus || item.status || "")
                      .toString()
                      .toUpperCase()}
                  </span>
                  ...
                  {(() => {
                    const claim = myClaimsMap && myClaimsMap[String(item.id)];
                    if (!claim) {
                      return (
                        <span className="inline-block px-3 py-1 text-xs rounded-full bg-red-50 text-red-700 font-medium">
                          UNCLAIMED
                        </span>
                      );
                    }
              ...
              <div className="mt-auto flex items-center gap-3">
                ...
                {/* Verify link: shown when user's claim exists and is in a verify-able status */}
                {(() => {
                  const claim = myClaimsMap && myClaimsMap[String(item.id)];
                    ...
                    return (
                      <div>
                        <Link
                          ...
                          className="px-3 py-2 text-sm bg-white text-gray-700 rounded-md border hover:bg-gray-50 transition"
                        >
                          Verify
                        </Link>
                      </div>
